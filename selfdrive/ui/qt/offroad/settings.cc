...
TogglesPanel::TogglesPanel(QWidget *parent) : QWidget(parent) {
  ...
  toggles.append(new ParamControl("IsUploadRawEnabled",
                                 "로그 데이터를 업로드 합니다.",
                                 "WiFi를 사용하는 동안 기본적으로 로그 및 영상을 업로드합니다. 활성화되지 않은 경우 my.comma.ai/useradmin에서 개별 로그를 업로드하도록 표시할 수 있습니다.",
  ...
  toggles.append(new ParamControl("EndToEndToggle",
                                   "\U0001f96c 차선이 없을 때 사용 버전 알파 (Alpha) \U0001f96c",
...
DevicePanel::DevicePanel(QWidget* parent) : QWidget(parent) {
  ...
  QPushButton *reset_calib_btn = new QPushButton("라이브파라메터 및 캘리브레이션 리셋");
  reset_layout->addWidget(reset_calib_btn);
  QObject::connect(reset_calib_btn, &QPushButton::released, [=]() {
    if (ConfirmationDialog::confirm("캘리브레이션 및 라이브파라메터 를 다시 설정 하시겠습니까?", this)) {
